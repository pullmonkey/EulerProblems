...
largest_prod_of_four = 0

20.times do |x| # don't have to bother with the last ones
  20.times do |y| # ditto
    val = grid[x][y].to_i
    next if val == 0 # the product would be 0, so next!!
    prods = []
    # just going to check down (which would be up in some cases), right (which would be left in some cases) and the 2 diags (TL to BR and TR to BL)
    # check down if we can go down (y<=16, taken care of by the iterator)
    prods << val * grid[x][y+1].to_i * grid[x][y+2].to_i * grid[x][y+3].to_i if y <= 16
    # check right if we can go right (x<=16, taken care of by the iterator)
    prods << val * grid[x+1][y].to_i * grid[x+2][y].to_i * grid[x+3][y].to_i if x <= 16
    # check diags (only these two to skip unnecessary checks) 
    prods << val * grid[x+1][y+1].to_i * grid[x+2][y+2].to_i * grid[x+3][y+3].to_i if x <= 16 and y <= 16 # TL to BR
    prods << val * grid[x-1][y+1].to_i * grid[x-2][y+2].to_i * grid[x-3][y+3].to_i if x >= 4  and y <= 16  # TR to BL
    largest_prod_of_four = prods.max if (prods.max || 0) > largest_prod_of_four
  end
end

puts largest_prod_of_four
